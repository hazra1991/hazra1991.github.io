...
* <details>
  ...
  - ✅ Yes, you can send the defined schema in JSON via HTTP (`Content-Type: application/json`), and compliant OTel backends should accept it.

  So — OTel is not only Protobuf. JSON is official, but Protobuf is preferred for performance.
...
<details>
  ...
  ```markdown
  ResourceSpans
  └── resource (describes service: attributes like service.name, sdk.version)
  └── scope_spans (groups by instrumentation library)
        └── spans[] (the actual spans)
              ...
              ├── links[] (span links across traces)
...
<details>
  <summary><strong>Here’s how the above look when serialized as JSON.[click to view]</strong></summary>
...
<details>
  ...
  └── scope_metrics
        └── metrics[]
              ...
              └── data (one of:)
                  • Gauge (instant value)
                  • Sum (counter / monotonic / non-monotonic)
                  • Histogram (bucketed distribution)
                  • ExponentialHistogram
                  • Summary (quantiles, deprecated)
...
<details>
  <summary><strong>Here’s how the above look when serialized as JSON.Metric (Histogram Example)[click to view]</strong></summary>
...
<details>
  ...
  └── scope_logs
      └── log_records[]
            ├── time_unix_nano
            ├── severity_number (enum: TRACE, DEBUG, INFO, WARN, ERROR, FATAL)
            ├── severity_text (string version, e.g., "ERROR")
            ├── body (actual log message, AnyValue type)
            ├── attributes[] (extra metadata)
            ├── trace_id (to link log → trace)
            ├── span_id (to link log → span)
